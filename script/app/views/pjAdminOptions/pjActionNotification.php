					...
					<form id="frmNotification" action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminOptions&amp;action=pjActionUpdate" method="post" class="form pj-form">
						...
						<div id="tabs">
							<ul>
								...
								<li><a href="#tabs-3"><?php __('menuPrintOrder');?></a></li>
							</ul>
							<div id="tabs-1">
								<?php
								pjUtil::printNotice(__('infoNotificationsTitle', true), __('infoNotificationsDesc', true)); 
								?>
												...
												if ($tpl['arr'][$i]['tab_id'] == 3 && (int) $tpl['arr'][$i]['is_visible'] === 1 && (strpos($tpl['arr'][$i]['key'], 'admin') === false) && (strpos($tpl['arr'][$i]['key'], 'email') > -1))
												{
													...
													<tr class="pj-table-row-odd<?php echo $rowClass; ?>" style="<?php echo $rowStyle; ?>">
														<td width="35%" valign="top">
															...
															<span class="fs10"><?php echo nl2br(__('opt_' . $tpl['arr'][$i]['key'].'_text', true, false)); ?></span>
														</td>
														<td valign="top">
															<?php
																	...
																	?>
																		<?php
																			foreach ($tpl['lp_arr'] as $v)
																			{
																				...
																				<p class="pj-multilang-wrap" data-index="<?php echo $v['id']; ?>" style="display: <?php echo (int) $v['is_default'] === 0 ? 'none' : NULL; ?>">
																					<span class="inline_block">
																						<textarea name="i18n[<?php echo $v['id']; ?>][<?php echo $tpl['arr'][$i]['key'] ?>]" class="pj-form-field" style="width: 400px; height: 500px;"><?php echo htmlspecialchars(stripslashes(@$tpl['arr']['i18n'][$v['id']][$tpl['arr'][$i]['key']])); ?></textarea>
																						<span class="pj-multilang-input"><img src="<?php echo PJ_INSTALL_URL . PJ_FRAMEWORK_LIBS_PATH . 'pj/img/flags/' . $v['file']; ?>" alt="" /></span>
																					</span>
																				</p>
																				<?php
							...
							<div id="tabs-2">
								<?php
								pjUtil::printNotice(__('infoNotifications2Title', true), __('infoNotifications2Desc', true)); 
								?>
								<div class="multilang"></div>
								<br/><br/>
								<fieldset class="fieldset white">
												...
												{											
													...
													<tr class="pj-table-row-odd<?php echo $rowClass; ?>" style="<?php echo $rowStyle; ?>">
														<td width="35%" valign="top">
															...
															<span class="fs10"><?php echo nl2br(__('opt_' . ( !in_array($tpl['arr'][$i]['key'], array('o_admin_email_confirmation', 'o_admin_email_payment', 'o_admin_email_cancel')) ? str_replace('admin_', '', $tpl['arr'][$i]['key']) : $tpl['arr'][$i]['key']) . '_text', true, false)); ?></span>
														</td>
														<td valign="top">
															<?php
															switch ($tpl['arr'][$i]['type'])
							...
							<div id="tabs-3">
								<?php
								pjUtil::printNotice(__('infoPrintTemplateTitle', true), __('infoPrintTemplateDesc', true)); 
								?>
								<div class="multilang"></div>
								<br/><br/>
								...
								<table class="pj-table" cellpadding="0" cellspacing="0" style="width: 100%">
									...
									<tbody>
										...
										for ($i = 0; $i < $count; $i++)
										{
											if ($tpl['arr'][$i]['tab_id'] == 7 && (int) $tpl['arr'][$i]['is_visible'] === 1)
											{
												$rowClass = NULL;
												$rowStyle = NULL;
												?>
												<tr class="pj-table-row-odd<?php echo $rowClass; ?>" style="<?php echo $rowStyle; ?>">
													...
													<td>
														...
														{
															case 'string':
																if(in_array($tpl['arr'][$i]['key'], array('o_print_order')))
																{
																?>
																	<?php
																		foreach ($tpl['lp_arr'] as $v)
